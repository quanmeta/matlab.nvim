M = {}
local _defaults = {
    matlab_dir = "/opt/MATLAB/R2023b/bin/matlab"
}
...
M.setup = function(user_opts)
    ...
    vim.api.nvim_create_user_command("MatlabCliRunFile", M.matlab_cli_run_current_file, {})
...
M.matlab_cli_run_current_file = function ()
  local buf = vim.api.nvim_get_current_buf()
  local file = vim.api.nvim_buf_get_name(buf)

  vim.cmd("write")

  if M.is_function_file(buf) then
    -- function 文件：调用函数名
    local fname = vim.fn.fnamemodify(file, ":t:r")
    M.matlab_cli_run_command(fname .. ";\n")
  else
    -- script 文件
    M.matlab_cli_run_command(
      string.format("run('%s');\n", file)
    )
  end
end
...
M.matlab_cli_toggle = function()
    -- 情况 1：从未打开过 Matlab（buffer 不存在）
    if not M._cli_buff or not vim.api.nvim_buf_is_valid(M._cli_buff) then
        M.matlab_cli_open()
        return
    end

    -- 情况 2：buffer 存在，但窗口已关闭 → 重新打开
    if not M._cli_win_handle or not vim.api.nvim_win_is_valid(M._cli_win_handle) then
        vim.cmd("vsplit")
        vim.cmd("vertical resize 50")

        M._cli_win_handle = vim.api.nvim_get_current_win()
        vim.api.nvim_win_set_buf(M._cli_win_handle, M._cli_buff)
        return
    end
    ...
    if cur_win == M._cli_win_handle then
        -- 3a：当前就在 Matlab 窗口 → 隐藏
        vim.api.nvim_win_hide(M._cli_win_handle)
        M._cli_win_handle = nil
    else
        -- 3b：当前在编辑区 → 跳转到 Matlab 窗口
        vim.api.nvim_set_current_win(M._cli_win_handle)
    end
end
